...
let inference_args =
  let generated_by =
    ...
    <?> "generated by"
  in
  let rule = ws_sep *> string "l:" *> inference_rule <?> "inference label" in
  lift2 (fun gb rule -> (gb, rule)) generated_by rule
  <|> lift2 (fun rule gb -> (gb, rule)) rule generated_by
...
let inference_line =
  ...
  >>= fun consequent ->
  inference_args >>= fun (generated_by, rule) ->
  return { constraint_id; premises; consequent; generated_by; rule }
  <* eol <?> "inference line"
